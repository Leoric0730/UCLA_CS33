...
return !((~(x+1)^x)|neg1); // Both Tmin and -1 satisfy the first condition, so
//remove -1 by or with it
}
//2
/*
 ...
 */
 int evenBits(void) {
 int word = 0x55; //85 with binary of 01010101
 word = word | word << 8; // make word a 16 bits 0101...
 return word | word << 16;} // make a int(32 bits) by combining word and it
 //shifting 16 bits
 //3
 /*
 ...
 */
 ...
 //a 32-bit 2's complement number can be represented as n-bit number if
 //its first (32-n) leading bits has the same value as its nth bit value.
 int move = 32 +(~n+1);
 ...
 */
 ...
 return (((y & ~x) | ~((x ^ y) | (x + ~y))) >> 31) & 1;
     ...
     */
     ...
     //what we need to do here is find the binary representation of abs x,
     //and OR it with its sign(negative with MSB as 1; positive with MSB as 0)
     int neg_mask = x >> 31;
     int x_sign = neg_mask << 31;
     int x_abs = (neg_mask ^ x) + (neg_mask & 1);
     return x_sign | x_abs;
     }
     //10
     /*
      ...
      */
      int isPower2(int x) {
      //a positive number is a power of 2 if leading bit is not 0 and x&(x-1)=0
      return !(x & (x + ~0)) & !(x>>31) & !!x;
      }
